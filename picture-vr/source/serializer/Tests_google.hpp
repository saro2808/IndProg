...
void recursive_TreeTest(TreeNode& preticted_tree, TreeNode& tree) {
    ASSERT_EQ(preticted_tree.data.rotation, tree.data.rotation);
    ASSERT_EQ(preticted_tree.data.move, tree.data.move);
    ASSERT_EQ(preticted_tree.data.scale, tree.data.scale);
    ASSERT_EQ(preticted_tree.data.id_texture, tree.data.id_texture);
    ASSERT_EQ(preticted_tree.data.id_name, tree.data.id_name);
    ASSERT_EQ(preticted_tree.data.id_type, tree.data.id_type);
    for (int i = 0; preticted_tree.children[i] && tree.children[i]; ++i) {
        recursive_TreeTest(*preticted_tree.children[i], *tree.children[i]);
    }
}

TEST(SerializerTest, TreeTest1) {
    ...
    ASSERT_EQ(unserilized_root->id_frame , root->id_frame);
    ASSERT_EQ(unserilized_root->time , root->time);
